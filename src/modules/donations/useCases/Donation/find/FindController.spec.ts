...
describe('Find Donation Controller', () => {
  const id = uuidV4();
  beforeAll(async () => {
    ...
    await connection.query(
      ...
      VALUES('1', 'Donation Type', 'now()', 'now()')`,
    );
  ...
  it('should be able to find a donation', async () => {
    ...
    const responseDonation = await request(app).post('/donations').send({
      name: 'Donation Name',
    ...
    const donationId = JSON.parse(responseDonation.text).id;

    const response = await request(app).get(`/donations/find?id=${donationId}`).send().set({
      Authorization: `Bearer ${token}`,
    });

    expect(response.body.id).toEqual(donationId);
    expect(response.body).toHaveProperty('id');
    expect(response.body.name).toEqual('Donation Name');
  });
});
