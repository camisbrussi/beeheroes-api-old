...
describe('Find Project Controller', () => {
  ...
  beforeAll(async () => {
    ...
    await connection.query(
      ...
      VALUES('${id}', 'Project Type', 'xxxxxx', '123456', 'organization@beeheroes.com', '1', '1', '${id}', 'now()', 'now()')`,
    );
    ...
    await connection.query(
      `INSERT INTO VOLUNTEERS(id, user_id, occupation_area_id, created_at, updated_at) 
  ...
  it('should be able to find a project', async () => {
    ...
    const responseEvaluation = await request(app).post('/evaluations').send({
      score: 5,
      description: 'xxx',
      subscription_id: id,
    }).set({
    ...
    const projectId = JSON.parse(responseEvaluation.text).id;

    const response = await request(app).get(`/evaluations/find?id=${projectId}`).send().set({
      Authorization: `Bearer ${token}`,
    });
