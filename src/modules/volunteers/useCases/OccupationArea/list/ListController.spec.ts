...
describe('List Occupation Area Controller', () => {
  beforeAll(async () => {
  ...
  it('should be able to list all categories', async () => {
    ...
    const response = await request(app).get('/occupationarea').set({
      Authorization: `Bearer ${refresh_token}`,
    });
    ...
    expect(response.body[0].name).toEqual('Occupation Area Supertest');
  });
});
