...
describe('List Volunteer Controller', () => {
  const id = uuidV4();
  beforeAll(async () => {
  ...
  it('should be able to list all volunteer', async () => {
    ...
    const response = await request(app).get('/volunteers').set({
      Authorization: `Bearer ${refresh_token}`,
    });
    ...
    expect(response.body[0].cpf).toEqual('11111');
  });
});
